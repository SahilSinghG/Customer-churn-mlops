...
class CustomerFeatures(BaseModel):
    ...
    MultipleLines: Optional[str] = Field("No", description="Multiple lines: Yes, No, or No phone service")
    InternetService: str = Field(..., description="Internet service: DSL, Fiber optic, or No")
    OnlineSecurity: Optional[str] = Field("No", description="Online security: Yes, No, or No internet service")
    OnlineBackup: Optional[str] = Field("No", description="Online backup: Yes, No, or No internet service")
    DeviceProtection: Optional[str] = Field("No", description="Device protection: Yes, No, or No internet service")
    TechSupport: Optional[str] = Field("No", description="Tech support: Yes, No, or No internet service")
    StreamingTV: Optional[str] = Field("No", description="Streaming TV: Yes, No, or No internet service")
    StreamingMovies: Optional[str] = Field("No", description="Streaming movies: Yes, No, or No internet service")
    ...
    PaymentMethod: str = Field(..., description="Payment method: Electronic check, Mailed check, Bank transfer, or Credit card")
    MonthlyCharges: float = Field(..., description="Monthly charges")
    TotalCharges: Optional[float] = Field(None, description="Total charges (calculated if not provided)")
    ...
    try:
        ...
        for customer in batch_request.customers:
            ...
            if customer_dict.get("TotalCharges") is None:
                customer_dict["TotalCharges"] = customer_dict["tenure"] * customer_dict["MonthlyCharges"]
